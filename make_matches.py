...
for i, row in papers.iterrows():
    ...
    for o, chunk in enumerate(text_chunks):
        ...
        metadatas.append({"id":row["paperID"],"type": "paper", "title":row["title"], 'contribution_type': row['contribution_type']})
...
for i, (distance, meta) in enumerate(zip(matches['distances'], matches['metadatas'])):
    for d, m in zip(distance, meta):
        data.append({"paperID": metadatas[i]['id'],"title":metadatas[i]['title'], 'contribution_type': metadatas[i]['contribution_type'], "personID": m['id'], "distance": d, 'first_name': m['first_name'], 'last_name': m['last_name']})
...
for reviewer in reviewers:
    for assignment in reviewer['assignments']:
        output.append({'personID':reviewer['personID'],'firstname':reviewer['firstname'],'name':reviewer['name'],'paperID':assignment['paperID'],'title':assignment['title'],'distance':assignment['distance'], 'contribution_type': assignment['contribution_type']})
